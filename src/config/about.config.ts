...
export const defaultAboutConfig = {
  // 🖼️ 首屏 · 身份锚定区
  hero: {
    avatar: bloggerConfig.avatar.src,
    avatarAlt: bloggerConfig.avatar.alt,
    name: bloggerConfig.name,
    handle: '@yu_wen',
...
export type AboutConfig = typeof defaultAboutConfig;
...
const userConfigModules = import.meta.glob('./.config/about.config.ts', { eager: true });
const aboutUserModule = userConfigModules['./.config/about.config.ts'] as Record<string, any> | undefined;
const userAboutConfig: Partial<AboutConfig> | undefined = aboutUserModule?.userAboutConfig;

export const aboutConfig: AboutConfig = userAboutConfig
  ? mergeConfig(defaultAboutConfig, userAboutConfig)
  : defaultAboutConfig;
