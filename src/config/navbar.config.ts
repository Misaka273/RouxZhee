...
import { siteOverviewConfig } from './site.config';
...
export interface NavLink {
  ...
  external?: boolean;
  // 📦 是否收纳到「更多」菜单
  //     collapse: true 则收纳 更多按钮里
  //     collapse: false 则展示在外面的胶囊内，默认false
  collapse?: boolean;
  // 📝 子菜单（支持多级嵌套）
  children?: NavLink[];
}
...
export const defaultNavbarConfig = {
  // 🏠 Logo 配置（从 SEO 与站点配置读取）
  logo: {
    // Logo 图片路径
    src: siteOverviewConfig.site.icon,
    // 网站名称
    ...
    links: [
      {
        name: 'GitHub',
        href:
          bloggerConfig.social.links.find((link) => link.name === 'GitHub')?.href ||
          'https://github.com/yuwenGueen/RouxZhee',
        hint: '查看本项目仓库', // 可自定义，空字符串则不显示
        // 🎨 SVG
